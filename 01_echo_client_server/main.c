...
static void _recvCallback(SSL* ssl)
{
    ...
    for (i=0; (i<200) && dtls_isAlive(ssl); i++)
    {
        ...
        writelen = SSL_write(ssl, buffer, readlen);
        if (dtls_checkSslWrite(ssl, buffer, writelen) != DTLS_OK)
        {
            derror("dtls_checkSslWrite failed");
            goto _END;
        }

        dprint("write back : %s", buffer);
...
static void _client(char* remote_ip, int remote_port)
{
    ...
    for (i=0; (i<200) && dtls_isAlive(client.ssl); i++)
    {
        ...
        readlen = SSL_read(client.ssl, buffer, sizeof(buffer));
        check = dtls_checkSslRead(client.ssl, buffer, readlen);
        if (check == DTLS_FAIL)
        {
            derror("dtls_checkSslRead failed");
            goto _END;
